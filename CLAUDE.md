...
Issue 与 PRD 以 GitHub issue 形式存在，用 `gh` CLI 操作；外部 PR 与 issue 共用同一套标签和状态。见 `docs/agents/issue-tracker.md`。

### Triage labels

五个标准角色映射到同名 GitHub 标签（`needs-triage`、`needs-info`、`ready-for-agent`、`ready-for-human`、`wontfix`）。见 `docs/agents/triage-labels.md`。

### Domain docs

单一上下文：仓库根一份 `CONTEXT.md` + `docs/adr/`。见 `docs/agents/domain.md`。

### Loop Engineering

`/loop-go <任务>` 循环派 builder 写代码、checker 跑全量检查，直到全部通过或触发停止条件。定义见 `.claude/agents/builder.md`、`.claude/agents/checker.md`、`.claude/commands/loop-go.md`；停止规则见本文档 `## Loop 停止规则`。
